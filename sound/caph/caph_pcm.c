...
static bool early_suspend_registered = FALSE;
#endif /* CONFIG_HAS_EARLYSUSPEND */
static struct pi_mgr_dfs_node dfs_node;
static char *audio_dfs = "audio_dfs";
static bool dfs_node_created = FALSE;
static bool during_playback = FALSE;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void audio_early_suspend(struct early_suspend *s);
...
static struct early_suspend audio_early_suspend_desc = {
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 5,
	.suspend = audio_early_suspend,
	.resume = audio_late_resume,
};
#endif /* CONFIG_HAS_EARLYSUSPEND */
...
static void audio_early_suspend(struct early_suspend *s)
{
	aTrace(LOG_ALSA_INTERFACE,"audio early suspend entered %s", __func__);
