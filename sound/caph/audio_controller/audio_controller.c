...
#define DELAY_HSPMU_ON 40*1000
#define DELAY_IHFPMU_ON 60*1000
#define DELAY_PMU_OFF 2*1000
#define DELAY_BEFORE_PMU_OFF 100*1000

static int wait_bb_on;
static int wait_hspmu_on = DELAY_HSPMU_ON;
static int wait_ihfpmu_on = DELAY_IHFPMU_ON;
static int wait_pmu_off = DELAY_PMU_OFF;
static int wait_before_pmu_off = 0;
...
static void AUDCTRL_SetAudioExtraVolume(void);
				 ...
				 bool force)
{
	...
	if (voiceCallMic != source || voiceCallSpkr != sink) {
		...
		AUDDRV_PathID_t *curTelephonyPathID = NULL;

		curTelephonyPathID = AUDDRV_GetTelephonyPath();

		aTrace(LOG_AUDIO_CNTLR, "%s: set audio profile only (app %d, mode %d)\n",
			__func__, app, mode);

		AUDDRV_SetAudioMode(mode, app,
			curTelephonyPathID->ulPathID,
			curTelephonyPathID->ul2PathID,
	...
	if(sExtraVol)
	{
		if(mode == AUDIO_MODE_HANDSET)
			mode = AUDIO_MODE_TTY;
		else if(mode == AUDIO_MODE_SPEAKERPHONE)
			mode = AUDIO_MODE_HAC;
	}
		...
		int volume, AUDIO_GAIN_FORMAT_t gain_format)
{
	...
	if (app >= AUDIO_APP_NUMBER) {
		aWarn(
				"AUDCTRL_SetTelephonySpkrVolume()"
...
{
	AudioApp_t currApp = AUDCTRL_GetAudioApp();
	aTrace(LOG_AUDIO_CNTLR, "%s currApp=%d, new app=%d", __func__, currApp, app);
...
void AUDCTRL_SetAudioMode(AudioMode_t mode, AudioApp_t app)
{
	...
	aTrace(LOG_AUDIO_CNTLR, "SetAudioMode: mode %d app %d", mode, app);
   	...
   	/* VT call routine go inside below code so it's just returned because of check condition*/
	if (AUDCTRL_InVoiceCall())
		if (app > AUDIO_APP_VOIP_INCOMM)
			return;
...
#if 0
	if (IsAudioDock && (mode == AUDIO_MODE_HEADSET))
	{
	    mode = AUDIO_MODE_HANDSFREE;
	    aTrace(LOG_AUDIO_CNTLR," mode separation for AudioDock");
	}
#endif
...
*/
	...
	if (bSuspendExtraVol) {
		bSuspendExtraVol = FALSE;
		AUDCTRL_SetAudioExtraVolume();
	}
}
			   ...
			   int vol_left, int vol_right, unsigned int pathID)
{
	...
	aTrace(LOG_AUDIO_CNTLR,
		"%s source %d sink %d format %d vol %d:%d path %d\n",
		__func__, source, sink, gain_format, vol_left, vol_right,
		pathID);
	...
	***/
	...
	if (gain_format == AUDIO_GAIN_FORMAT_FM_RADIO_DIGITAL_VOLUME_TABLE) {
		...
		if (vol_left < 0 ||
			...
			aWarn("%s FM volume %d out of range\n",
				__func__, vol_left);
		...
		/*If FM volume comes before start, no input would be available*/
	...
	if (mute == TRUE) {
		...
		if (sink == AUDIO_SINK_VIBRA)
			...
			if (mixInCh != CSL_CAPH_SRCM_INCHNL_NONE) {
				...
				csl_srcmixer_setMixInGain(mixInCh, mixer, mixInGain, mixInGainR);
			   ...
			   AUDIO_SINK_Enum_t sink, unsigned int pathID)
{
	...
	if (bInVoiceCall) {
		...
		aTrace(LOG_AUDIO_CNTLR,
		"%s audio play in voice call, no path change.", __func__);
	...
	/* make sure currAudioMode is updated if not in voice call */
	if (AUDCTRL_InVoiceCall() == FALSE || pPath->src_sampleRate != AUDIO_SAMPLING_RATE_44100)
		AUDCTRL_SaveAudioMode(GetAudioModeBySink(sink));
			...
			AUDIO_SINK_Enum_t sink, unsigned int pathID)
{
	...
	switch (sink)
	{
		case AUDIO_SINK_LOUDSPK:
			AUDCTRL_SetAudioMode_ForMusicPlayback(
				AUDCTRL_GetModeBySpeaker(curr_spk), pathID, FALSE);
	AUDCTRL_SetAudioMode_ForMusicPlayback(
		GetAudioModeBySink(sink), pathID, FALSE);
			break;
		case AUDIO_SINK_HEADSET:
	...
	*pPathID = pathID;
	...
	if (config.sink == CSL_CAPH_DEV_DSP) {
		...
		if (!AUDCTRL_InVoiceCall()) {
			...
			RPC_SetProperty(RPC_PROP_AUDIO_MODE,
				(UInt32)(((int)AUDCTRL_GetAudioApp()) *
				AUDIO_MODE_NUMBER + AUDCTRL_GetAudioMode()));
#endif
		}
			  ...
			  AUDIO_SAMPLING_RATE_t sr, unsigned int *pPathID)
{
	...
	if (bInVoiceCall && source != AUDIO_SOURCE_I2S) {
		if (sr != AUDIO_SAMPLING_RATE_48000) {
			...
			curTelephonyPathID = AUDDRV_GetTelephonyPath();
			/*in call mode, return the UL path*/
			*pPathID = curTelephonyPathID->ulPathID;
...
/****************************************************************************
*
* Function Name: AUDCTRL_SetAudioExtraVolume
*
* Description:   set audio profile for extra volume
*
****************************************************************************/
static void AUDCTRL_SetAudioExtraVolume(void)
{
	aTrace(LOG_AUDIO_CNTLR, "%s:: enable extra volume: %d\n",
		__func__, sExtraVol);

	if (!bInVoiceCall) {
		aTrace(LOG_AUDIO_CNTLR, "%s:: skip extra volume in non-call state: %d\n",
			__func__, bInVoiceCall);
		return;
	}

	if (sExtraVol) {
		if (voiceCallSampleRate == AUDIO_SAMPLING_RATE_16000)
			sForcedApp = AUDIO_APP_VOICE_CALL_WB_EXTRAVOL;
		else
			sForcedApp = AUDIO_APP_VOICE_CALL_EXTRAVOL;
				...
				int arg1, int arg2, int arg3, int arg4)
{
	...
	case AUDCTRL_HW_CFG_DMA:
		csl_audio_render_set_dma_size(arg1, arg2);
		break;
	...
	case AUDCTRL_HW_CFG_DSPMUTE:
		AUDCTRL_SetTelephonySpkrMute(AUDIO_SINK_UNDEFINED, arg1);
		isHwCfgDspMute = arg1;
		aTrace(LOG_AUDIO_CNTLR,"isHwCfgDspMute=%d\n",isHwCfgDspMute);
		break;
	...
	case AUDCTRL_HW_CFG_DHAVEQ1:
		VoIF_setDhaVoiceEq(0, arg1, arg2, arg3, arg4);
		break;
	case AUDCTRL_HW_CFG_DHAVEQ2:
		VoIF_setDhaVoiceEq(1, arg1, arg2, arg3, arg4);
		break;
#endif
#endif
	case AUDCTRL_HW_CFG_EXTRA_VOLUME:
		...
		if (bInPlayback) {
			aTrace(LOG_AUDIO_CNTLR, "%s:: wait until stopping playback: %d\n",
				__func__, playbackPathID);
			bSuspendExtraVol = TRUE;
			break;
		}
...
static void setExternAudioGain(AudioMode_t mode, AudioApp_t app)
{
    ...
    if (IsAudioDock && mode == AUDIO_MODE_HEADSET && (app == AUDIO_APP_MUSIC || app == AUDIO_APP_TONE))
    {
        mode = AUDIO_MODE_HANDSFREE;
        aTrace(LOG_AUDIO_CNTLR, "%s audio dock mode %d, app %d\n", __func__, mode, app);
    }
...
#endif
	case AUDIO_MODE_HANDSFREE: // Audio Dock
		if (!IsAudioDock && mode == AUDIO_MODE_HANDSFREE)
				break;
