...
static struct usb_configuration android_config_driver = {
	...
	.bMaxPower	= CONFIG_USB_GADGET_VBUS_DRAW/2, /* 500ma max */
};
...
static void android_work(struct work_struct *data)
{
	...
	spin_lock_irqsave(&cdev->lock, flags);
		...
		printk("USBD][%s] Gadget Enumeration Success \n",__func__);
	}
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled && cdev) {
		...
		// BST jean.ding@ 2012.11.28 modify bcdDevice field of
		// USB Descriptor for all samsung model
...
{									\
	...
	return strlcpy(buffer, buf, buf[size - 1] == '\n' ? size : size + 1);\
}									\
static DEVICE_ATTR(field, S_IRUGO | S_IWUSR, field ## _show, field ## _store);
