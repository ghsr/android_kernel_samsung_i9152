...
static int lightval_logcount = 500;
static int als_raw;
...
int cm3663_i2c_read(struct cm3663_data *cm3663, u8 addr, u8 *val)
{
	...
	if ((client == NULL) || (!client->adapter)) {
                pr_err("%s: i2c read failed client error\n", __func__);        
		return -ENODEV;
	}
	...
	while (retry--) {
		err = i2c_transfer(client->adapter, msg, 1);
                debug("%s: i2c read addr 0x%x: %d, val=%d", __func__, addr, err, *val);
		if (err >= 0)
			return err;
	}
...
int cm3663_i2c_write(struct cm3663_data *cm3663, u8 addr, u8 val)
{
	...
	if ((client == NULL) || (!client->adapter)) {
        	pr_err("%s: i2c write failed client error\n", __func__);
		return -ENODEV;
	}
	...
	while (retry--) {
		err = i2c_transfer(client->adapter, msg, 1);
        	debug("%s: i2c write at addr 0x%x: %d", __func__, addr, err); 
		if (err >= 0)
			return err;
	}
	...
	struct device_attribute *attr, char *buf)
{
	...
	adc = als_raw; //lightsensor_get_alsvalue(cm3663);
				...
				const char *buf, size_t size)
{
	...
	if (err < 0)
        ...
        printk(KERN_INFO "[CM3663] poll_delay_store : new_delay=%lld\n", new_delay);
				  ...
				  const char *buf, size_t size)
{
	...
	if (sysfs_streq(buf, "1"))
        ...
        printk(KERN_INFO "[CM3663] light_enable_store : new_value=%d\n", new_value);
...
static DEVICE_ATTR(prox_cal, S_IRUGO | S_IWUSR, 
...
static int cm3663_setup_irq(struct cm3663_data *cm3663)
{
	...
	printk(KERN_INFO "[CM3663] request_irq success IRQ_NO:%d, GPIO:%d", irq, cm3663->i2c_client->irq);
			  ...
			  const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "[CM3663] %s start \n", __func__);	
	...
	i2c_set_clientdata(client, cm3663);
        ...
        /*PROXY_EN*/
	if (gpio_request(cm3663->pdata ->power_gpio, "PROXY_EN")) {
		printk(KERN_ERR "Request GPIO_%d failed!\n", cm3663->pdata ->power_gpio);
	}
	cm3663->pdata->proximity_power(1);   
	...
	/*Initialisation of PS_ALS_INT of proximity sensor*/
	if (gpio_request(cm3663->i2c_client->irq, "PS_ALS_INT")) {
		printk(KERN_ERR "Request GPIO_%d failed!\n", cm3663->i2c_client->irq);
	}	
	gpio_direction_input(cm3663->i2c_client->irq);
	...
	cm3663->proximity_dev = device_create(sensors_class,
	...
	if (device_create_file(cm3663->proximity_dev, &dev_attr_state) < 0) {
	...
	cm3663->switch_cmd_dev = device_create(sensors_class,
	...
	if (device_create_file(cm3663->switch_cmd_dev, &dev_attr_adc) < 0) {
		pr_err("%s: could not create device file(%s)!\n", __func__,
	...
	cm3663->pdata->proximity_power(0);   
        printk(KERN_INFO "[CM3663] gpio_get_value of GPIO(%d) is %d\n",cm3663->pdata ->power_gpio, 
            gpio_get_value(cm3663->pdata ->power_gpio));
...
err_light_device_create:
    	device_remove_file(cm3663->proximity_dev, &dev_attr_prox_cal);
...
static int cm3663_i2c_remove(struct i2c_client *client)
{
	...
	device_remove_file(cm3663->switch_cmd_dev, &dev_attr_adc);
	device_remove_file(cm3663->proximity_dev, &dev_attr_prox_cal);    
	device_remove_file(cm3663->proximity_dev, &dev_attr_prox_avg);
	device_remove_file(cm3663->proximity_dev, &dev_attr_state);
	device_destroy(sensors_class, 0);
...
static int __init cm3663_init(void)
{
	...
	if(IS_ERR(VCC_PDA_2_8_V)){
        ...
        {
            printk(KERN_ERR "[CM3663] can not get prox_regulator (ALS_VDD_2.8V) \n");
        }
        ret = regulator_is_enabled(prox_regulator);
        printk(KERN_INFO "[CM3663] regulator_is_enabled : %d\n", ret);
