						...
						if (tch.abs[CY_TCH_MAJ + j] == 90)
						{
							...
							input_report_abs(md->input, ABS_MT_TOUCH_MAJOR, 0x90);		// 144
						      ...
						      input_report_abs(md->input, ABS_MT_WIDTH_MAJOR,	tch.abs[CY_TCH_MAJ + j]);
                                            input_report_abs(md->input, ABS_MT_TOUCH_MAJOR, tch.abs[CY_TCH_MAJ + j]);
                                       }
					}
				}
...
#if defined(PALM_ACTION) || defined(HAND_BLADE_ACTION)
	input_set_abs_params(md->input, ABS_MT_PALM, 0, 1, 0, 0);
	input_set_abs_params(md->input, ABS_MT_ANGLE, CY_MIN_ANGLE, CY_MAX_ANGLE, 0, 0);
	input_set_abs_params(md->input, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(md->input, ABS_MT_WIDTH_MINOR, 0, 255, 0, 0);
#endif
