...
#define POLLRATE_POLL_FORCE	10
#define POLLRATE_IDLE_INIT	160
#define POLLRATE_IDLE_FORCE	40
#define POLLRATE_IDLE		100
#define POLLRATE_RETRY		500
#define POLLRATE_ADC		20
#define POLLRATE_CUTOFF		2000
#define POLLRATE_DIETEMP	1000
...
#define HICAL_FACT_INIT		100
#define LOCAL_FACT		80

#define GUARD_OFF_NORM		10
#define GUARD_ON_INIT		10
#define GUARD_OFF_INIT		5
...
enum {
	BCMPMU_FG_ST_FB_CAL = (1U << 0),
	BCMPMU_FG_ST_LB_CAL = (1U << 1),
};
...
struct bcmpmu_em {
	...
	int bvcap_len;
	struct bcmpmu_cutoff_map *cutoffcal;
	int cutoffcal_len;
	struct bcmpmu_currcap_map *eoccal;
	int eoccal_len;
	int charge_1c_rate;
	int eoc;
	int eoc_state;
	...
	s64 fg_capacity_full_recharge;
	s64 fg_capacity_full_backup;
	int support_fg;
	int support_chrg_maint;
	s64 fg_capacity;
	s64 fg_capacity_temp;
	int fg_cap_cal;
	int fg_1st_fb_cal;
	int fg_force_cal;
	int fg_lowbatt_cal;
	...
	int dtemp_monitor;
	int dtemp_threshold;
	int dtemp_hysteresis;
	int dtemp_throttle;
	int die_dbg_temp;
	bool usb_ov;
	int pre_eoc_min_curr;
	int max_batt_ov;
...
static void save_fg_status(struct bcmpmu *bcmpmu, int bit, int status)
{
	int ret = 0;
	int temp;
	ret = bcmpmu->read_dev(bcmpmu,
			PMU_REG_FG_ST,
			&temp,
			bcmpmu->regmap[PMU_REG_FG_ST].mask);
	if (ret != 0) {
		pr_em(ERROR, "%s failed to read fg status.\n", __func__);
		return;
	}
	if (status)
		temp = temp | bit;
	...
	ret = bcmpmu->write_dev(bcmpmu,
		...
		temp,
		bcmpmu->regmap[PMU_REG_FG_ST].mask);
	pr_em(FLOW, "%s, ret=%d, st=0x%X, bit=0x%X\n",
		__func__, ret, temp, bit);
	return;
}

static int get_fg_status(struct bcmpmu *bcmpmu, int bit)
{
	int ret = 0;
	int data;
	ret = bcmpmu->read_dev(bcmpmu,
	...
	data = data & bit;
	pr_em(FLOW, "%s, status=%d, bit=0x%X\n", __func__, data, bit);
	return data;
}
...
static int get_fg_rate(struct bcmpmu *bcmpmu)
{
	int ret = 0;
	int data = 0;
	ret = bcmpmu->read_dev(bcmpmu,
			...
			bcmpmu->regmap[PMU_REG_FG_CIC].mask);
	if (ret != 0)
		pr_em(ERROR, "%s failed to read fg rate.\n", __func__);
...
static int get_esr_ocv(struct bcmpmu_em *pem, int slope, int offset)
{
	int ocv;
	s64 temp;
	s64 temp1;
	temp = 1000 + (slope * pem->batt_curr / 1000);
	temp1 = 1000 * pem->batt_volt - offset * pem->batt_curr;
	pr_em(DATA, "%s, temp =%lld, temp1=%lld\n",
		__func__, temp, temp1);
	if(temp != 0)
	ocv = div64_s64(temp1, temp);
	pr_em(DATA, "%s, ocv =%d, slope=%d, offset=%d\n",
		__func__, ocv, slope, offset);
	return ocv;
}

static int get_esr(struct bcmpmu_em *pem, int slope, int offset){
	
	int dividend;
	int divisor;
	int esr=0;

	dividend = 1000 * offset + (pem->batt_volt * slope);
	
	divisor =  (pem->batt_curr * slope / 1000) +1000;
	pr_em(DATA, "%s, dividend=%d, divisor=%d, \n",
		__func__, dividend, divisor);
	if (divisor !=0)
		esr = div64_s64(dividend,divisor);
	return esr;
}	
/**
 * get_ocv() - Calculate battery OCV based on battery current, terminal voltage,
 *             and ESR data.
 * @pem:	Pointer to bcmpmu_em structure
 *
 * Using battery current (pem->batt_curr), terminal voltage (pem->batt_volt),
 * and ESR data for the current temperature zone (pem->fg_zone), calculate
 * the battery Open-Circuit Voltage (OCV).
 *
 * Note:
 * - If temperature control is off (pem->fg_support_tc = 0), by default the
 *   temperature zone will be FG_TMP_ZONE_MAX = FG_TMP_ZONE_p20.
 * - If pem->fg_zone_ptr = NULL, a static ESR (pem->esr) will be used for the
 *   calculation.
 *
 * Function returns OCV in mV.
 *
 */
static int get_ocv(struct bcmpmu_em *pem)
{
	int slope;
	int offset;
	int ocv;
	...
	min_volt = pem->cutoffcal[pem->cutoffcal_len-1].volt;
	esr_vf_lvl = pem->max_volt;
	max_volt = pem->max_volt;
	ocv = 0;

	pr_em(FLOW, "%s, volt:%d,current:%d, min_volt:%d, max_volt:%d\n",
	__func__, pem->batt_volt, pem->batt_curr, min_volt, max_volt);
	if (!pem->fg_zone_ptr) {
		pr_em(FLOW, "%s, FG zone info not defined volt\n", __func__);
		ocv = pem->batt_volt - (pem->esr * pem->batt_curr)/1000;
		return ocv;
		...
		ocv = get_esr_ocv(pem, slope, offset);
		if ((ocv > min_volt) &&
			(ocv < (pem->fg_zone_ptr[pem->fg_zone].esr_vl_lvl)))
			return ocv;

			slope = pem->fg_zone_ptr[pem->fg_zone].esr_vm_slope;
			offset = pem->fg_zone_ptr[pem->fg_zone].esr_vm_offset;
		ocv = get_esr_ocv(pem, slope, offset);
		if ((ocv < (pem->fg_zone_ptr[pem->fg_zone].esr_vm_lvl)) &&
			(ocv >= (pem->fg_zone_ptr[pem->fg_zone].esr_vl_lvl)))
			return ocv;
		...
		ocv = get_esr_ocv(pem, slope, offset);
		if ((ocv < (pem->fg_zone_ptr[pem->fg_zone].esr_vh_lvl)) &&
			(ocv >= (pem->fg_zone_ptr[pem->fg_zone].esr_vm_lvl)))
			return ocv;
			...
			esr_vl= pem->fg_zone_ptr[pem->fg_zone].esr_vl_offset + 
					(pem->fg_zone_ptr[pem->fg_zone].esr_vl_lvl *
					pem->fg_zone_ptr[pem->fg_zone].esr_vl_slope / 1000);
			esr_vl = (esr_vl * 105) / 100;
			esr_vf= pem->fg_zone_ptr[pem->fg_zone].esr_vf_offset + 
					(esr_vf_lvl *
					pem->fg_zone_ptr[pem->fg_zone].esr_vf_slope / 1000);
			esr_vf = (esr_vf * 95) / 100;
			
		    pr_em(DATA,"%s, ocv_vl=%d, ocv_vm=%d, ocv_vh=%d, ocv_vf=%d\n",
				__func__, pem->fg_zone_ptr[pem->fg_zone].esr_vl_lvl,
				pem->fg_zone_ptr[pem->fg_zone].esr_vm_lvl,
				pem->fg_zone_ptr[pem->fg_zone].esr_vh_lvl,
				esr_vf_lvl);
			pr_em(DATA,"%s,esr_vl=%d, esr_vf=%d\n",__func__, esr_vl,esr_vf);
			
			slope = pem->fg_zone_ptr[pem->fg_zone].esr_vf_slope;			
			offset = pem->fg_zone_ptr[pem->fg_zone].esr_vf_offset;
			esr = get_esr(pem, slope, offset);			
			if((esr >= esr_vf) && (esr <= esr_vl)){
				ocv = get_esr_ocv(pem, slope, offset);
				pr_em(DATA,"%s,ocv=%d, ocv_vf=%d\n",__func__,ocv,
					(esr_vf_lvl * 101)/100);
				if (ocv <= (esr_vf_lvl * 101)/100)				
					return ocv;			
			}
			slope = pem->fg_zone_ptr[pem->fg_zone].esr_vh_slope;
			offset = pem->fg_zone_ptr[pem->fg_zone].esr_vh_offset;
			...
			if((esr >= esr_vf) && (esr <= esr_vl)){
				ocv = get_esr_ocv(pem, slope, offset);
				pr_em(DATA,"%s,ocv=%d, ocv_vf=%d\n",__func__,ocv,
					((pem->fg_zone_ptr[pem->fg_zone].esr_vh_lvl) * 101) / 100);
				if (ocv <= ((pem->fg_zone_ptr[pem->fg_zone].esr_vh_lvl) * 101) / 100)				
					return ocv;
				}
			slope = pem->fg_zone_ptr[pem->fg_zone].esr_vm_slope;
			offset = pem->fg_zone_ptr[pem->fg_zone].esr_vm_offset;
			...
			if((esr >= esr_vf) && (esr <= esr_vl)){
				ocv = get_esr_ocv(pem, slope, offset);
				pr_em(DATA,"%s,ocv=%d, ocv_vf=%d\n",__func__,ocv,
					((pem->fg_zone_ptr[pem->fg_zone].esr_vm_lvl) * 101) / 100);
				if (ocv <= ((pem->fg_zone_ptr[pem->fg_zone].esr_vm_lvl) * 101) / 100)
					return ocv;
				}
			slope = pem->fg_zone_ptr[pem->fg_zone].esr_vl_slope;
			offset = pem->fg_zone_ptr[pem->fg_zone].esr_vl_offset;
			...
			if((esr >= esr_vf) && (esr <= esr_vl)){
				ocv = get_esr_ocv(pem, slope, offset);
				pr_em(DATA,"%s,ocv=%d, ocv_vf=%d\n",__func__,ocv,
					((pem->fg_zone_ptr[pem->fg_zone].esr_vl_lvl) * 101) /100);
				if (ocv <= ((pem->fg_zone_ptr[pem->fg_zone].esr_vl_lvl) * 101) /100)
					return ocv;		
				}
		}		
		if (ocv < min_volt) {
			pr_em(FLOW, "OCV below min voltage!!\n");
			ocv = min_volt;
		} else if (ocv > max_volt) {
			pr_em(FLOW, "OCV above max voltage!!\n");
			ocv = max_volt;
		}
			return ocv;
		}
...
static void update_fg_capacity(struct bcmpmu_em *pem, int capacity)
{
	...
	pem->fg_capacity_temp = pem->fg_capacity;
...
static ssize_t cutoffcal_tbl_show(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	struct bcmpmu *bcmpmu = dev->platform_data;
	...
	int i;
	ssize_t count;
	count = sprintf(buf, "Cutoff volt map table\n");
	for (i = 0; i < pem->cutoffcal_len; i++)
		count += sprintf(buf+count, "volt=%d, capacity=%d\n",
				pem->cutoffcal[i].volt,
				pem->cutoffcal[i].cap);
	return count;
}

static ssize_t eoccal_tbl_show(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	struct bcmpmu *bcmpmu = dev->platform_data;
	...
	int i;
	ssize_t count;
	count = sprintf(buf, "EOC curr map table\n");
	for (i = 0; i < pem->eoccal_len; i++)
		count += sprintf(buf+count, "curr=%d, capacity=%d\n",
				pem->eoccal[i].curr,
				pem->eoccal[i].cap);
	return count;
}
...
static ssize_t
die_temp_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct bcmpmu *bcmpmu = dev->platform_data;
	struct bcmpmu_em *pem = bcmpmu->eminfo;
	return sprintf(buf, "die_dbg_temp=%d\n", pem->die_dbg_temp);
}

static ssize_t
die_temp_set(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct bcmpmu *bcmpmu = dev->platform_data;
	struct bcmpmu_em *pem = bcmpmu->eminfo;
...
static ssize_t fg_curr_ua_show(struct device *dev,
				  ...
				  char *buf)
{
	struct bcmpmu *bcmpmu = bcmpmu_em->bcmpmu;
	struct bcmpmu_em *pem = bcmpmu->eminfo;
...
static DEVICE_ATTR(die_dbg_temp, 0644, die_temp_show, die_temp_set);
...
static DEVICE_ATTR(eoccal, 0644, eoccal_tbl_show, NULL);
static DEVICE_ATTR(cutoffcal, 0644, cutoffcal_tbl_show, NULL);
#ifdef SAMSUNG_DFT_SOC_RETURN_NODE
...
static struct attribute *bcmpmu_em_attrs[] = {
	...
	&dev_attr_eoc.attr,
	&dev_attr_eoccal.attr,
	&dev_attr_cutoffcal.attr,
	NULL
};
...
static bool is_charging_stop_hw(struct bcmpmu_em *pem)
{
	bool ret;
	ret = pem->usb_ov;
	return ret;
...
/**
 * soc_lookup() - Look up the battery SoC from the voltage-capacity map
 * @pem:	Pointer to bcmpmu_em structure
 * @ocv:	Battery Open-Circuit Voltage (mV)
 *
 * Given the Open Circuit Voltage (OCV) of the battery, look up the battery
 * State of Charge (SoC) from the voltage-capacity map. Returns battery SoC (%)
 */
static int soc_lookup(struct bcmpmu_em *pem, int ocv)
{
	int i = 0;
	...
	if (ocv >= vcmap[0].volt)
		cap = vcmap[0].cap;
	else if (ocv <= vcmap[len - 1].volt)
		cap = vcmap[len - 1].cap;
	else {
		for (i = 0; i < len - 1; i++) {
			if ((ocv <= vcmap[i].volt) &&
			    (ocv > vcmap[i+1].volt)) {
				index = ((vcmap[i].volt - ocv) * 1000)/
...
static int update_adc_readings(struct bcmpmu_em *pem)
{
	...
	if (pem->dtemp_monitor) {
		req.sig = PMU_ADC_DIETEMP;
		req.tm = PMU_ADC_TM_HK;
		req.flags = PMU_ADC_RAW_AND_UNIT;
		ret |= pem->bcmpmu->adc_req(pem->bcmpmu, &req);
		if (ret == 0)
			die_temp = req.cnv;
		pr_em(FLOW, "%s, cnv=%d, dietemp%d, ret=%d\n",
			__func__, req.cnv, die_temp, ret);
...
static void pre_eoc_check(struct bcmpmu_em *pem)
{
	...
	if ((pem->batt_capacity < 90) ||
		...
		(pem->batt_curr > pem->pre_eoc_min_curr) ||
		(pem->batt_curr < 0)) {
		pem->eoc_factor = 0;
		return;
	}

	for (i = 0; i < pem->eoccal_len - 1; i++) {
		if ((pem->batt_curr <= pem->eoccal[i].curr) &&
			(pem->batt_curr > pem->eoccal[i+1].curr)) {
			index = i;
			break;
		}
	}
	if (index >= 0)
		capacity = pem->eoccal[index].cap;
	else
...
static int update_eoc(struct bcmpmu_em *pem)
{
	...
	if (capacity == 100) {
		if (pem->fg_1st_fb_cal == 0) {
			pem->fg_1st_fb_cal = 1;
			save_fg_status(pem->bcmpmu,
				BCMPMU_FG_ST_FB_CAL, 1);
		}
...
static void clear_cal_state(struct bcmpmu_em *pem)
{
	int i;
	for (i = 0; i < pem->cutoffcal_len; i++)
		pem->cutoffcal[i].state = 0;
	pem->cutoff_delta = 0;
	pem->capacity_cutoff = -1;

	for (i = 0; i < pem->eoccal_len; i++)
		pem->eoccal[i].state = 0;
	pem->eoc_count = 0;
...
static int pre_cutoff_check(struct bcmpmu_em *pem)
{
	...
	for (i = 0; i < pem->cutoffcal_len; i++) {
		if ((pem->cutoffcal[i].state == 0) &&
			(pem->batt_volt <= pem->cutoffcal[i].volt)) {
			index = i;
			break;
		}
	}
	...
	if (pem->cutoff_chk_cnt > pem->cutoff_count_max) {
		pem->cutoff_chk_cnt = 0;
		capacity = pem->cutoffcal[i].cap;
		pem->cutoffcal[i].state = 1;
	}
	pem->cutoff_cal_index = index;
...
static int get_fg_guard_upper(struct bcmpmu_em *pem)
{
	if (pem->fg_1st_fb_cal == 1)
		return pem->fg_guard;
	else
		return GUARD_ON_INIT;
}

static int get_fg_guard_lower(struct bcmpmu_em *pem)
{
	if (pem->fg_1st_fb_cal == 1)
		return GUARD_OFF_NORM;
	else
		return GUARD_OFF_INIT;
}

static void update_hical_factor(struct bcmpmu_em *pem,
	int capacity, int capacity_v)
{
	if (capacity != 0)
		pem->high_cal_factor =
			(capacity_v - capacity) * 100 / capacity;
	else
		pem->high_cal_factor = 0;
	if (pem->fg_1st_fb_cal == 1) {
		...
		if (pem->high_cal_factor > HICAL_FACT_INIT)
			pem->high_cal_factor = HICAL_FACT_INIT;
		else if (pem->high_cal_factor < -HICAL_FACT)
			pem->high_cal_factor = -HICAL_FACT;
	}
}
...
static int update_batt_capacity(struct bcmpmu_em *pem, int *cap)
{
	...
	if (pem->fg_comp_mode_last != pem->fg_comp_mode) {
		pem->fg_comp_mode_last = pem->fg_comp_mode;
		pem->fg_capacity_temp = pem->fg_capacity;
	}
...
*/
	...
	if (first_time == 0) {
		...
		pem->avg_oc_volt = (pem->avg_oc_volt *
				(AVG_LENGTH - 1))/AVG_LENGTH +
				pem->oc_volt / AVG_LENGTH;
	}

	capacity_v = soc_lookup(pem, pem->oc_volt);
	...
	/*
	 * capacity = (fg_capacity / fg_capacity_full * 100) + 0.5
	 * 0.5 is added for proper rounding in the division
	 */
		capacity64 = pem->fg_capacity * 100 + pem->fg_capacity_full / 2;
		capacity = div64_s64(capacity64, pem->fg_capacity_full);

	if ((pem->fg_comp_mode != 0) &&
		(!is_charger_present(pem))) {
		fg_result_temp = (fg_result * 1000)/pem->fg_temp_fact;
		pem->fg_capacity_temp += fg_result_temp;

		if (pem->fg_capacity_temp >= pem->fg_capacity_full)
			pem->fg_capacity_temp = pem->fg_capacity_full;
		if (pem->fg_capacity_temp <= 0)
			pem->fg_capacity_temp = 0;

		capacity64 = pem->fg_capacity_temp * 100
			+ pem->fg_capacity_full / 2;
		capacity_temp = div64_s64(capacity64, pem->fg_capacity_full);

		pr_em(DATA, "%s, fg_t=%d, fg_cap_t=%lld, cap_t=%d\n", __func__,
			fg_result_temp, pem->fg_capacity_temp, capacity_temp);
	} else
	...
	if ((!is_charger_present(pem)) &&
		...
		if ((!is_charger_present(pem)) &&
			...
			(pem->fg_1st_fb_cal == 0)) {
			pem->fg_1st_fb_cal = 1;
		save_fg_status(pem->bcmpmu, BCMPMU_FG_ST_FB_CAL, 1);
		}
	...
	*cap = capacity;
	if (calibration == 0) {
		if ((pem->fg_comp_mode != 0) &&
			(!is_charger_present(pem))) {
			capacity_temp = ((capacity * capacity_temp) + 50) / 100;
			*cap = capacity_temp +
				((100 - capacity) * capacity + 50) / 100;
		if ((capacity > 0) && (*cap == 0))
			*cap = capacity;
		}
	}
...
err:
	...
	if (!is_charger_present(pem)) {
		...
		else if (pem->die_temp >
			(pem->dtemp_threshold - pem->dtemp_hysteresis))
			pem->mode = MODE_DIETEMP;
		else
			pem->mode = MODE_CHRG;
	}
	return calibration;
...
static void init_charge(struct bcmpmu_em *pem)
{
	pem->vfloat = pem->max_vfloat;
	pem->bcmpmu->set_vfloat(pem->bcmpmu, pem->vfloat);
	pem->icc_fc = pem->chrgr_curr;
...
static int get_update_rate(struct bcmpmu_em *pem)
{
	...
	switch (pem->mode) {
		...
		case MODE_CHRG:
			...
			break;
		case MODE_DIETEMP:
			if (pem->dtemp_monitor == 0)
				rate = POLLRATE_CHRG;
			else
				rate = POLLRATE_DIETEMP;
			break;
...
static int update_charge_current_limit(struct bcmpmu_em *pem)
{
	...
	if (pem->die_dbg_temp != 0)
		pem->die_temp = pem->die_dbg_temp;

	if (bcmpmu->set_icc_fc_lmt) {
		if (pem->die_temp >= pem->dtemp_threshold)
			bcmpmu->set_icc_fc_lmt(bcmpmu,
				pem->dtemp_throttle);
		else if (pem->die_temp < (pem->dtemp_threshold
			- pem->dtemp_hysteresis))
			bcmpmu->set_icc_fc_lmt(bcmpmu, -1);
	}
	if (pem->die_temp <
		(pem->dtemp_threshold - pem->dtemp_hysteresis))
		return 0;
...
static int charging_algorithm(struct bcmpmu_em *pem)
{
	int charge_zone;
	struct bcmpmu *bcmpmu = pem->bcmpmu;
	int ret = 0;
	...
	if (is_charger_present(pem)) {
		if (pem->charge_state == CHRG_STATE_MAINT) {
			...
			/* if there is external charger configured for the
			platform we do not need to check the validity of
			PMU_ENV_USB_VALID, as PMU's MBC block is disabled and
			this bit will never be set */
			if (pem->support_ext_chrgr ||
				bcmpmu->get_env_bit_status(bcmpmu,
			...
			if (bcmpmu->get_env_bit_status(bcmpmu,
				...
				bcmpmu->chrgr_usb_en(bcmpmu, 1);
			pr_em(FLOW,
					"%s, enable charge again.\n",
				__func__);
		}
			if (pem->non_pse_charging) {
				if (pem->icc_fc != pem->chrgr_curr) {
					init_charge(pem);
					bcmpmu->chrgr_usb_en(bcmpmu, 1);
				}
			}
		...
		if (capacity == 100) {
			...
			if ((pem->batt_status != POWER_SUPPLY_STATUS_CHARGING)
				...
				pr_em(FLOW, "%s, transition to charging\n",
					__func__);
				psy_changed = 1;
			} else if (is_charging_stop_hw(pem) == true) {
				propval.intval =
				    POWER_SUPPLY_STATUS_NOT_CHARGING;
				ps->set_property(ps, POWER_SUPPLY_PROP_STATUS,
						 &propval);
				pem->batt_status =
				    POWER_SUPPLY_STATUS_NOT_CHARGING;
				pr_em(FLOW,
				      "%s, charger over voltage transition to not charging\n",
				      __func__);
				psy_changed = 1;
			}
		}
	}
	propval.intval = pem->batt_volt*1000;
...
static void change_fg_full_capacity(struct bcmpmu_em *pem, int* capacity)
{
    pr_info("%s, %d\n", __func__, pem->batt_recharge);
    if(pem->batt_recharge == POWER_SUPPLY_STATUS_FULL)
    {
        s64 fg_capacity;
        pr_info("%s,before fg_capacity_full_recharge=%lld, fg_capacity_full=%lld, capacity=%d\n",
                __func__, pem->fg_capacity_full_recharge, pem->fg_capacity_full, *capacity);

        fg_capacity  = pem->fg_capacity_full * (*capacity);
        fg_capacity = div_s64(fg_capacity, 100);
        pem->fg_capacity_full_recharge = fg_capacity;
        
        *capacity = 100;
        pem->batt_capacity = 100;
    	pr_info("%s,after fg_capacity_full_recharge=%lld, fg_capacity_full=%lld\n",
                __func__, pem->fg_capacity_full_recharge, pem->fg_capacity_full);
    }
}
...
static void em_algorithm(struct work_struct *work)
{
	...
	if (poll_count > 0) {
		...
		pem->oc_volt = get_ocv(pem);
		capacity = soc_lookup(pem, pem->oc_volt);
	...
	if (pem->mode == MODE_POLL) {
		...
		if (pem->init_poll == 1) {
			if ((pem->cap_init > 0) &&
				...
				pem->fg_1st_fb_cal = 0;
				save_fg_status(pem->bcmpmu,
					BCMPMU_FG_ST_FB_CAL, 0);
			}
			bcmpmu_usb_get(pem->bcmpmu,
	...
	if (update_charge_current_limit(pem) != 0) {
		if ((get_seconds() - pem->time) * 1000 < POLLRATE_CHRG) {
			pem->mode = MODE_DIETEMP;
			schedule_delayed_work(&pem->work,
				msecs_to_jiffies(get_update_rate(pem)));
			return;
	...
	if (charging_algorithm(pem) != 0) {
		pem->mode = MODE_IDLE;
		pr_em(FLOW, "%s, Retry charging.\n", __func__);
		schedule_delayed_work(&pem->work,
			msecs_to_jiffies(get_update_rate(pem)));
		return;
	}
	...
	change_fg_full_capacity(pem, &capacity);
	...
	pr_em(REPORT, "EM: capacity=%d, volt=%d, curr=%d, temp=%d, die=%d\n",
		capacity, pem->batt_volt, pem->batt_curr,
		pem->batt_temp, pem->die_temp);
	...
	if (pem->mode == MODE_CUTOFF)
		...
		wd_tapper_set_timeout(TAPPER_DEFAULT_TIMEOUT);
#endif
#endif
		...
		unsigned long event, void *para)
{
	...
	case BCMPMU_CHRGR_EVENT_CHGR_DETECTION:
		...
		if (is_charger_present(pem)) {
            ...
            if(pem->batt_recharge == POWER_SUPPLY_STATUS_UNKNOWN)
			{
                if(pem->fg_capacity_full != pem->fg_capacity_full_backup)
                    pr_em(REPORT, "%s, change to orig full capacity, current full=%lld\n",
                        __func__, pem->fg_capacity_full);
                
				pem->fg_capacity_full = pem->fg_capacity_full_backup;
				update_fg_capacity(pem, pem->batt_capacity);
			}
		}
		else
		{
			if(pem->batt_recharge == POWER_SUPPLY_STATUS_FULL)
			{
                ...
                if(pem->fg_capacity_full != pem->fg_capacity_full_recharge)
                    pr_em(REPORT, "%s, change to new full capacity, new full=%lld\n",
                        __func__, pem->fg_capacity_full_recharge);
                
                pem->fg_capacity_full = pem->fg_capacity_full_recharge;
                update_fg_capacity(pem, pem->batt_capacity);
			}
		}
	...
	case BCMPMU_EXTCHRGR_EVENT_DETECTION:
		pem->ext_chrgr = *(int *)para;
		if (is_charger_present(pem))
			pem->fg_cap_cal = 0;
	...
	case BCMPMU_FG_EVENT_FGC:
		...
		capacity_v = soc_lookup(pem, req.cnv);
...
static int __devinit bcmpmu_em_probe(struct platform_device *pdev)
{
	...
	pem->support_ext_chrgr = pdata->support_ext_chrgr;
	...
	if (pdata->fg_capacity_full)
	{
		...
		pem->fg_capacity_full_recharge= pdata->fg_capacity_full;
        	pem->fg_capacity_full_backup = pdata->fg_capacity_full;
	}
	else
	{
		pem->fg_capacity_full = 1000*3600;
		pem->fg_capacity_full_recharge= 1000*3600;
        	pem->fg_capacity_full_backup = 1000*3600;
	}
	...
	if (pdata->cutoff_cal_map) {
		pem->cutoffcal = pdata->cutoff_cal_map;
		pem->cutoffcal_len = pdata->cutoff_cal_map_len;
	} else {
		pem->cutoffcal = &cutoff_cal_map[0];
		pem->cutoffcal_len = ARRAY_SIZE(cutoff_cal_map);
	}

	if (pdata->eoc_cal_map) {
		pem->eoccal = pdata->eoc_cal_map;
		pem->eoccal_len = pdata->eoc_cal_map_len;
	} else {
		pem->eoccal = &eoc_cal_map[0];
		pem->eoccal_len = ARRAY_SIZE(eoc_cal_map);
	}
	...
	if (pdata->max_batt_ov)
		pem->max_batt_ov = pdata->max_batt_ov;
	else
		pem->max_batt_ov = MAX_BATT_OV;
	if (pdata->pre_eoc_min_curr)
		pem->pre_eoc_min_curr = pdata->pre_eoc_min_curr;
	else
		pem->pre_eoc_min_curr = PRE_EOC_MIN_CURR;

	if (pdata->max_vfloat)
		pem->max_volt = pdata->max_vfloat;
	else
		pem->max_volt = MAX_VOLT;
	...
	ret = bcmpmu_add_notifier(BCMPMU_CHRGR_EVENT_USBOV, &pem->nb);
	if (ret) {
		pr_em(INIT, "%s, failed on usbov notifier, err=%d\n",
		      __func__, ret);
		goto err;
	}
	...
	ret = bcmpmu_add_notifier(BCMPMU_EXTCHRGR_EVENT_DETECTION, &pem->nb);
	if (ret) {
		pr_em(INIT, "%s, failed to register ext ac notifier, err=%d\n",
			__func__, ret);
		goto err;
	}
	pem->ext_chrgr = bcmpmu->usb_accy_data.ext_chrgr_present;
	bcmpmu->em_reset = em_reset;
	bcmpmu->em_reset_status = em_reset_status;

	pem->dtemp_monitor = pdata->support_die_temp_monitor;
	if (pem->dtemp_monitor) {
		pem->dtemp_threshold = pdata->die_temp_threshold;
		pem->dtemp_hysteresis = pdata->die_temp_hysteresis;
		pem->dtemp_throttle = pdata->die_temp_throttle;
	}
	pem->die_dbg_temp = 0;
	...
	schedule_delayed_work(&pem->work, msecs_to_jiffies(0));
...
err:
	...
	ret = bcmpmu_remove_notifier(BCMPMU_CHRGR_EVENT_USBOV, &pem->nb);
	...
	ret = bcmpmu_remove_notifier(BCMPMU_CHRGR_EVENT_CHRG_STATUS,
		&pem->nb);
	ret = bcmpmu_remove_notifier(BCMPMU_EXTCHRGR_EVENT_DETECTION,
		&pem->nb);
...
subsys_initcall_sync(bcmpmu_em_init);
